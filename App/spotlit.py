...
firebaseConfig = {
  ...
  'appId': "1:YOUR_FIREBASE_CONFIG:web:YOUR_FIREBASE_CONFIG",
...
if choice == '🔓 SignIn':
    ...
    if not login:
      ...
      with col_a:
        fire_message1 = '<p style="font-family:sans-serif; color:Orange; font-size: 15px;">Hey, spotter! Did something come up your radar!</p>'
    ...
    if login:
      ...
      bio = option_menu(
        ...
        styles = {
          ...
          "nav-link-selected": {"font-size": "25px", "background-color": "#E6E6FA"},
        }
      )
...
# Shore
        ...
        if button_clicked: #add_post:
          if selected == "Crackle":
              ...
              for intent in intents['intents']:
                if tag == intent['tag']:
                  ...
                  if intent['tag'] == "insult":
                      ...
                      # #st.write(" 👌 Hey there .. spotter, keep up!")
            ...
            if post['Post'] != "":
              report = []
              test = str(post['Post']) #post['Post']
              user_input = f"Is this '{test}' morally and socially inappropriate online."
          ...
          if button_clicked: #add_post:
            if selected == "Crackle":
              ...
              if prediction == "anger":
                ...
                if nothing == "True":
                  st.success(f"👃 Ethics Judge")
                  
                  user_input = f"Is this '{test}' morally inappropriate online?"
                                   
                  #st.markdown("----")
                  #ethics_box = st.empty()
                  #ethics_box.markdown(f' {} ')
                  report = []
                  ...
                  yes = re.search(r'Yes', s)
                  if yes:
                    st.write("🤨 Spotter, mind your ethics !")
                  else:
                    st.write("👍 Spotter, great ethics !")
                  ##############################################################
                  ##############################################################
                ...
                st.warning(f"🎨 Emotion Type")
                st.write(f"{spotshot} _{prediction}_: {emoji_icon}")
                st.info("📐 Emotion Level")
                st.write("🎯 Meter: {:.2f} %".format(np.max(probability)*100))
                
                st.success("💡 Tips & Advice")                 
                            
                user_input = f"Specifically, tell me three socially better ways to say {test} in the tone of emotion {i_prediction} "
                #st.markdown("----")
                ai_box = st.empty()
                starter = ""
                starter = "📝..."
            ...
            for i in range(3):
              ...
              user_input = f"Give me a new inspiring quotes from arbitrary historical figures!"
              #st.markdown("----")
              info_box = st.empty()
